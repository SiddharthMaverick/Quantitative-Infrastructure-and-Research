...
class Strategy:
    ...
    def onMarketData(self, marketData):
        ...
        target_time = datetime.strptime("2024-03-19 13:00:00", "%Y-%m-%d %H:%M:%S")
        expiry_date = datetime.strptime("2024-03-20 00:00:00", "%Y-%m-%d %H:%M:%S")
        ...
        if current_time >= target_time and self.x == 0 and marketData["Symbol"] == "MARK:BTCUSDT":
            ...
            put_strike = self.find_closest_strike(self.x * 0.95)
            call_strike = self.find_closest_strike(self.x * 1.05)
    ...
    def sendOrderToExchange(self, symbol, side, quantity, price):
        print(symbol," ",side," ",quantity," ",price)
    ...
    def calculatePnL(self):
        self.netPnL = self.totalPremiumReceived - sum([(self.closePrices[pos["Symbol"]][-1]) * pos['quantity'] for pos in self.openPositions])
